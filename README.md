...
- 注意一下，此项目使用了彭迁写的插件"vite-plugin-web-config"，该插件的作用是，自动配置跨域代理，包括本地的（就是vite.config.ts）里的，还有服务器上的（会在打包后的文件夹dist里自动生成web.config文件，注意测试服打包和正式服打包会生成对应的不同的，要分别放在对应的服务器里）
- '/@'已经按习惯改成'@'（'/@'有时候工具会报红）
- 增加了unocss配置
- SvgIcon代码修改过
