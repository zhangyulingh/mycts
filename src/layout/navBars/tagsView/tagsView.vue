...
import {routerUrls} from "@/router/route"
...
const refreshCurrentTagsView = async (fullPath: string) => {
  ...
  for (let i = 0; i < state.tagsViewList.length; i++) {
    const v = state.tagsViewList[i]
    v.transUrl = transUrlParams(v)
    if (v.transUrl && v.transUrl === transUrlParams(v)) {
      item = v
      break
    } else if (v.path === decodeURIPath) {
      item = v
      break
    }
  }
...
onBeforeMount(() => {
  ...
  mittBus.on("openShareTagsView", () => {
    if (getThemeConfig.value.isShareTagsView) {
      router.push(routerUrls.URL_Home)
      state.tagsViewList = []
