...
export function formatFlatteningRoutes(arr: any) {
	...
	for (let i = 0; i < arr.length; i++) {
		if (arr[i].children && !arr[i].meta.subRouter) {
...
//路由免登录白名单
const whiteList = ["login", "barcodeScan"]
...
router.beforeEach(async (to, from, next) => {
	...
	if (whiteList.includes(<string>to.name) && !token) {
		next()
	} else {
		if (!token) {
			next(`/login?redirect=${to.path}&params=${JSON.stringify(to.query ? to.query : to.params)}`)
			Session.clear()
		} else if (token && whiteList.includes(<string>to.name)) {
